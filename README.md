...
your PATH; without it, `node node/proxy.mjs` does the same thing. (When it is published,
`npx hero-memory-proxy` becomes the whole quickstart — the launcher package under `npm/` is ready.)
